...
if __name__ == '__main__':
    ...
    ahs_src_dir = r"D:\geo_data\ms.monina\airborne\ahs\2007-07-02_kalmthoutse_heide"
    ahs_plt_src = r"D:\work\ms.monina\wp5\kalmthoutse_heide\2007-07-02_ahs\ahs_strip_plot_id_linkage_unique.txt"
    ahs_bad_src = r"D:\work\ms.monina\wp5\kalmthoutse_heide\2007-07-02_ahs\ahs_bad_bands.txt"
    plt_shp_src = r"D:\work\ms.monina\wp5\kalmthoutse_heide\field\releve_686_plots.shp"
    plot_id_field = 'ID'

    tgt_dir = r"D:\work\ms.monina\wp5\kalmthoutse_heide\2007-07-02_ahs\spectra"
    tgt_file = "2007-07-02_ahs_kalmthoutse_heide_spectra.txt"

    neighborhood = 1
    
    # retrieving links between flight strips and plot ids
    image_plt_links = retrieve_image_plot_links(ahs_plt_src)
    # retrieving image files
    image_data_files = glob.glob(os.path.join(ahs_src_dir, "*.img"))
    # setting bands
    ahs_bands = set(range(1, 61))
    # retrieving bad bands from specified file
    if os.path.isfile(ahs_bad_src):
        ahs_bad_bands = [int(b) for b in open(ahs_bad_src).readlines()[1].split("\t")]
    else:
        ahs_bad_bands = list()
    # retrieving good bands by calculating the difference between all 
    # bands and those that have been declared bad
    ahs_good_bands = list(ahs_bands.difference(ahs_bad_bands))
    ...
    # caching plots
    cached_plots = ogr_utils.cache_locations(plt_ly, [plot_id_field])

    # setting up output
    output = list()
    
    # iterating over a selection of strip ids
    for strip_id in [1, 2, 3, 4, 5, 6]:
        ...
        for cp in sorted(cached_plots, key = itemgetter('attributes')):
            plot_id = cp['attributes'][plot_id_field]
        ...
        spectra = gdal_utils.extract_spectra(image_data[strip_id], strip_locations, verbose = True, neighborhood = neighborhood, bad_bands = ahs_bad_bands)
        ...
        for strip_location, spectrum in zip(strip_locations, spectra):
            plot_id = strip_location['attributes'][plot_id_field]
            # checking whether we're in some nxn-neighborhood, i.e. retrieved
        ...
        # finally adding header lines
            ...
            # otherwise the header is much easier to build
                ...
                strip_output.insert(0, "\t".join(("plot_id", "strip_id", "x", "y", "\t".join([str(k) for k in ahs_good_bands]))))
