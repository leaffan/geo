...
class TriangleWrapper():
    ...
    def apply_triangle(self, remote=False, polygon=''):
        ...
        if self.verbose:
            print(
                "+ Preparing and creating polygon input data for Triangle...")
    ...
    def write_poly_file(self, tgt_file=''):
        ...
        for i in range(len(self.segments)):
            # creating segment lines (segment id, endpoint,
            # endpoint, boundary marker)
            output.append(
                "%d %d %d" % (i + 1, self.segments[i][0], self.segments[i][1]))

        # working on hole definition section
        # checking whether source polygon is a MultiPolygon
        if hasattr(self.py, 'geoms'):
            # creating Triangle representation of the source MultiPolygon's
            # holes creating hole header (number of holes)
            hole_cnt = 0
            ...
            for single_py in self.py:
                for inner_ring in single_py.interiors:
                    ...
                    output.append(
                        "%d %f %f" % (h + 1, inner_py_pt.x, inner_py_pt.y))
                    h += 1
        # if source polygon is a simple polygon
            ...
            for i in range(len(self.py.interiors)):
                ...
                # guaranteed to be within the hole
                inner_py_pt = Polygon(
                    self.py.interiors[i]).representative_point()
                output.append(
                    "%d %f %f" % (i + 1, inner_py_pt.x, inner_py_pt.y))
    ...
    def execute_remote_triangle(self, ssh_cfg_file, verbose=False):
        """
        ...
        self.remote_poly_src = rsc.upload_file(
            self.poly_src, self.remote_data_dir)
        if verbose:
        ...
        if verbose:
            ...
            print("+ Triangle standard output:\n%s" % "\t" + "".join(
                ["\t%s" % s for s in so]).strip())
        if se:
            print("+ Triangle error output:\n%s" % "".join(
                ["\t%s" % s for s in se]).strip())
            try:
                self.cleanup()
        ...
        rsc.download_file(
            os.path.splitext(self.remote_poly_src)[0] + ".1.node",
            os.path.splitext(self.poly_src)[0] + ".1.node")
        rsc.download_file(
            os.path.splitext(self.remote_poly_src)[0] + ".1.ele",
            os.path.splitext(self.poly_src)[0] + ".1.ele")

        if self.verbose:
            print("+ Cleaning up remotely...")
        # creating and executing command tp remove temporarily created files
        rm_cmds = [
            "rm %s" % (os.path.splitext(self.remote_poly_src)[0] + ".*")]
    ...
    def read_node_file(self, node_src=''):
        ...
        # opening source file
        fo = open(node_src).readlines()

        # retrieving node count
        node_cnt = int(fo.pop(0).split()[0])
        print("%d nodes to read from %s" % (node_cnt, node_src))
    ...
    def read_ele_file(self, ele_src=''):
        ...
        # opening source file
        fo = open(ele_src).readlines()

        # retrieving triangle count
        triangle_cnt = int(fo.pop(0).split()[0])
        print("%d triangles to read from %s" % (triangle_cnt, ele_src))
        ...
        for line in fo:
            ...
            # splitting line
            tokens = line.split()
            # retrieving triangle id, and node indices constructing
            # the triangle
            ...
            # NB: pay attention NOT to use the -z command line switch that
            # alters Triangle's way of enumerating elements starting with zero
            aa = self.nodes[a - 1]
            bb = self.nodes[b - 1]
            cc = self.nodes[c - 1]
            # creating triangle
    ...
    def convert_poly_data(self):
        ...
        if hasattr(self.py, 'geoms'):
            vertex_cnt = 1
            for single_py in self.py:
                vertex_cnt = self.traverse_linear_ring(
                    single_py.exterior, vertex_cnt)
        # coounting vertices of a simple polygon's outline
        else:
            vertex_cnt = self.traverse_linear_ring(self.py.exterior)
        # counting vertices of all the multipolygon's holes adding the result
        # to the previously retrievd vertex count
        if hasattr(self.py, 'geoms'):
            for single_py in self.py:
                for inner_ring in single_py.interiors:
                    vertex_cnt = self.traverse_linear_ring(
                        inner_ring, vertex_cnt)
        # counting vertices of all a simple polygon's holes adding the result
        # to the previously retrievd vertex count
        else:
    ...
    def create_temp_file(self, content):
        ...
        tmp_fd, tmp_name = tempfile.mkstemp(
            prefix='tw_', suffix=self.POLY_SUFFIX)
        tgt = os.fdopen(tmp_fd, 'w')
    ...
    def prepare_remote_execution(self, ssh_cfg_file, remote_data_dir=''):
        """
        Prepare remote execution of Triangle by identifying a configuration
        file containing SSH connection information and an optional data
        directory that will contain the Triangle input and output data.
        """
        ...
        if os.path.isfile(ssh_cfg_file):
            ...
            print(
                "Couldn't find SSH configuration file '%s'..." % ssh_cfg_file)
    ...
    def build_triangle_cmd(self, remote=False):
        ...
        """
        if remote:
            self.triangle_bin = '"' + './' + "/".join(
                (self.TRIANGLE_BIN_DIR, self.TRIANGLE_BIN)) + '"'
            self.cmd = " ".join((
                self.triangle_bin, self.COMMAND_ARGUMENTS,
                '"' + self.remote_poly_src + '"'))
        else:
            self.triangle_bin = '"' + os.path.join(
                self.TRIANGLE_BIN_DIR, self.TRIANGLE_BIN) + '"'
            self.cmd = " ".join((
                self.triangle_bin, self.COMMAND_ARGUMENTS,
                '"' + self.poly_src + '"'))
    ...
    def retrieve_shared_edges(self, shared_nodes):
        ...
        for k in sorted(shared_nodes.keys()):
            ...
            for l in range(0, len(entry)):
                for m in range(l + 1, len(entry)):
                    if (entry[l], entry[m]) not in self.shared_edges:
    ...
    def cleanup(self):
        ...
        for tmp_name in self.tmp_files:
            ...
            for suffix in (
                self.POLY_SUFFIX, self.NODE_SUFFIX, self.ELE_SUFFIX
            ):
